...
describe('MatchTopStats', () => {
    ...
    it('renders summary info for first innings', () => {
        ...
        expect(screen.getByRole('heading', { name: /A vs B/i })).toBeInTheDocument();
    ...
    it('renders player names correctly', () => {
        ...
        render(<MatchTopStats match={match} teamAPlayers={mockTeamAPlayers} teamBPlayers={mockTeamBPlayers} />);
    ...
    it('renders recent events excluding announcements', () => {
        ...
        expect(within(ballsContainer).queryByText('Hidden')).not.toBeInTheDocument();
    ...
    it('renders latest announcement', () => {
        const match = {
            ...
            innings1Summary: { runs: 10, wickets: 0, overs: 1 },
            recentEvents: [
                { type: 'announcement', meta: { text: 'Rain Delay' }, createdAt: new Date().toISOString() }
            ],
            maxOvers: 20
        };
        render(<MatchTopStats match={match} />);
