...
describe('ScorerPanel', () => {
    ...
    it('records a run when clicking 4 button using socket', async () => {
        ...
        await waitFor(() => expect(mockEmit).toHaveBeenCalledWith('score:event', {
            matchId: '123',
            event: { type: 'runs', runs: 4 }
        }));

        // Should also call fetch for persistence
        expect((global as any).fetch).toHaveBeenCalledWith(expect.stringContaining('/api/matches/123/score'), expect.any(Object));
    ...
    it('falls back to fetch if socket is not connected', async () => {
        (getSocket as jest.Mock).mockReturnValue({ ...mockSocket, connected: false });
        const onUpdate = jest.fn();
        ...
        expect(mockEmit).not.toHaveBeenCalledWith('score:event', expect.any(Object));
