...
export async function POST(
  ...
  // TypeScript null check (ensureHasRole would have thrown if session was null)
  ...
  if (!winner || !decision) {
    return NextResponse.json(
      { error: "Winner and decision are required" },
      { status: 400 },
    );
  }
  ...
  if (winner !== match.teamA && winner !== match.teamB) {
    return NextResponse.json(
      { error: "Winner must be one of the playing teams" },
      { status: 400 },
    );
  }
  ...
  if (decision !== "bat" && decision !== "bowl") {
    return NextResponse.json(
      { error: "Decision must be 'bat' or 'bowl'" },
      { status: 400 },
    );
  }
