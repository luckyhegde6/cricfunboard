...
export async function POST(
  ...
  if (userRole !== "admin" && !isAssigned) {
    return NextResponse.json(
      { error: "Forbidden: You are not assigned to score this match" },
      { status: 403 },
    );
  }
  ...
  const _client = mongoose.connection; // or mongoose
  ...
  try {
    await sessionDb.withTransaction(async () => {
      ...
      const agg = await MatchEvent.aggregate([
        ...
        },
        {
          $group: {
            ...
            totalRuns: { $sum: "$runs" },
            totalWickets: {
              $sum: { $cond: [{ $eq: ["$type", "wicket"] }, 1, 0] },
            },
            totalBalls: {
              $sum: {
                $cond: [{ $in: ["$type", ["dot", "runs", "wicket"]] }, 1, 0],
              },
            },
            lastEvents: {
              $push: {
      ...
      const updateField =
        match.currentInnings === 1 ? "innings1Summary" : "innings2Summary";
      ...
      if (ev.meta?.currentStriker)
        updateOps.$set["currentBatters.striker"] = ev.meta.currentStriker;
      if (ev.meta?.currentNonStriker)
        updateOps.$set["currentBatters.nonStriker"] = ev.meta.currentNonStriker;
      if (ev.meta?.currentBowler)
        updateOps.$set.currentBowler = ev.meta.currentBowler;

      resultMatch = await MatchModel.findByIdAndUpdate(matchId, updateOps, {
        new: true,
        session: sessionDb,
      }).lean();
    });
