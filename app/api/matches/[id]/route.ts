...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const [teamA, teamB] = await Promise.all([
      TeamModel.findOne({ name: match.teamA }).select("players captainId viceCaptainId").lean(),
      TeamModel.findOne({ name: match.teamB }).select("players captainId viceCaptainId").lean()
    ]);
    ...
    const initBatting = () => ({ runs: 0, balls: 0, fours: 0, sixes: 0, dismissal: "" });
    const initBowling = () => ({ overs: 0, balls: 0, maidens: 0, runs: 0, wickets: 0 });

    // Fetch all events for detailed processing
    const allEvents = await MatchEvent.find({ matchId: id }).sort({ createdAt: 1 }).lean();
    ...
    allEvents.forEach((ev: any) => {
      ...
      if (ev.type === "wicket" && ev.batsman) {
        ...
        let text = "";
        if (ev.wicketType === "bowled") text = `b ${allEvents.find((e: any) => e._id === ev._id)?.bowler || "Bowler"}`;
        else if (ev.wicketType === "caught") text = `c ${ev.fielder || "Fielder"} b ${ev.bowler || "Bowler"}`;
    ...
    Object.values(scorecard.innings1.bowling).forEach((b: any) => b.overs = Math.floor(b.balls / 6) + (b.balls % 6) / 10);
    Object.values(scorecard.innings2.bowling).forEach((b: any) => b.overs = Math.floor(b.balls / 6) + (b.balls % 6) / 10);
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!matchToCheck) return NextResponse.json({ error: "Not found" }, { status: 404 });
