    ...
    <div className="space-y-8">
      <section className="bg-gradient-to-tr from-white to-slate-50 rounded-lg p-8 shadow-sm">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div>
            <h1 className="text-3xl sm:text-4xl font-extrabold leading-tight">Local Cricket Scoreboard</h1>
          ...
          <div className="hidden md:block w-64">
            <div className="bg-white border rounded p-4 text-center">
              ...
              <div className="mt-2 text-2xl font-semibold text-slate-800">{matches.filter((m: any) => m.status === "live").length}</div>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {matches.length ? matches.map((m: any) => <MatchCard key={m._id} match={m} />) : (
            // skeletons
            Array.from({ length: 3 }).map((_, i) => <div key={i} className="h-40 bg-white rounded shadow animate-pulse" />)
          )}
        </div>
      </section>
    </div>
