...
export default function ScorePage({
  ...
  const [_activeTab, _setActiveTab] = useState("info");
  const [showScoreBuilder, setShowScoreBuilder] = useState(true);

  // Modal States
  const [showBatsmanModal, setShowBatsmanModal] = useState(false);
  const [showBowlerModal, setShowBowlerModal] = useState(false);
  const [batsmanRole, setBatsmanRole] = useState<"striker" | "nonStriker">(
    "striker",
  );
  ...
  useEffect(() => {
    ...
    id, // Optimistically update or fetch fresh
    // If payload is full match object, use it. But usually it might be partial or raw DB doc.
    // Our GET /api/matches/[id] returns enriched data (players etc).
    // The socket payload from events/route.ts is the raw match doc.
    // So we should probably fetchMatch() to get enriched data, OR manually merge.
  ...
  useEffect(() => {
    ...
    fetchMatch();
  }, [id, session, status, router, fetchMatch]);
          ...
          <div className="flex items-start gap-4">
            <div className="flex-1">
              ...
              <p className="text-yellow-700 dark:text-yellow-400 mb-4">
                You are not assigned as the scorer for this match. Only the
                assigned scorer or an admin can access the scorer panel.
              </p>
              <Link
                href={`/match/${id}`}
  ...
  const currentSummary =
    match.currentInnings === 1 ? match.innings1Summary : match.innings2Summary;
  ...
  const availableBowlers = (bowlingTeamPlayers || []).filter(
    (p: any) => p.playerId !== (match as any).currentBowler,
  );

  // Extract current stats from scorecard
  const currentInningsData =
    match.currentInnings === 1
      ? (match as any).scorecard?.innings1
      : (match as any).scorecard?.innings2;
      ...
      <div className="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 sticky top-0 z-10">
        <div className="max-w-3xl mx-auto px-4">
          <div className="flex items-center h-16">
            <Link
              href="/"
              className="mr-4 p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-full transition-colors"
            >
            ...
            <div className="ml-auto flex items-center gap-3">
              {isAdmin && (
                <div className="flex items-center gap-2 bg-slate-100 dark:bg-slate-700 px-2 py-1 rounded-lg">
                  ...
                  >
                    <span
                      className={`absolute top-0.5 left-0.5 w-3 h-3 bg-white rounded-full transition-transform ${showScoreBuilder ? "translate-x-4" : "translate-x-0"}`}
                    />
                  </button>
                </div>
              )}
              <span
                className={`px-2 py-1 rounded text-xs font-bold uppercase ${match.status === "live" ? "bg-red-100 text-red-600" : "bg-slate-100 text-slate-600"}`}
              >
                {match.status}
              </span>
            </div>
