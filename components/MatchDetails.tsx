...
export default function MatchDetails({ match, isCompleted = false }: MatchDetailsProps) {
    const statusColors = {
    ...
    return (
        <div className={`bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden ${isCompleted ? "opacity-60" : ""}`}>
            <div className="p-6">
                {/* Match Header */}
                <div className="flex items-start justify-between mb-4">
                    ...
                    {/* Status Badge */}
                    <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium capitalize ${statusColors[match.status]}`}>
                        {match.status === "live" && (
                ...
                {/* Score Summary */}
                {match.summary && (match.status === "live" || match.status === "completed") && (
                    <div className="bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-700/30 dark:to-slate-800/30 rounded-lg p-4 border border-slate-200 dark:border-slate-600">
                        <div className="flex items-center justify-between">
                            <div>
                                <div className="text-sm text-slate-600 dark:text-slate-400 mb-1">Current Score</div>
                                <div className="text-3xl font-bold text-slate-900 dark:text-white">
                                    {match.summary.runs}/{match.summary.wickets}
                                </div>
                            </div>
                            <div className="text-right">
                                <div className="text-sm text-slate-600 dark:text-slate-400 mb-1">Overs</div>
