...
export default function TeamFixtures({ teamId, teamName }: TeamFixturesProps) {
    ...
    if (loading) {
        return (
            <div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 p-6">
                <h2 className="text-xl font-semibold text-slate-900 dark:text-white mb-4">Fixtures</h2>
    ...
    const pastMatches = matches.filter(m => m.status === "completed" || m.status === "abandoned");
...
function MatchTile({ match, canEdit, isPast = false }: { match: Match; canEdit: boolean; isPast?: boolean }) {
    ...
    return (
        ...
        >
            ...
            <div className="flex items-center justify-between text-xs">
                ...
                <span className={`px-2 py-0.5 rounded-full text-xs font-medium capitalize ${match.status === "live"
