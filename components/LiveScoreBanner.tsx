...
export default function LiveScoreBanner() {
  ...
  const strikerName = match.currentBatters?.striker
    ? getPlayerName(match.currentBatters.striker)
    : null;
  const nonStrikerName = match.currentBatters?.nonStriker
    ? getPlayerName(match.currentBatters.nonStriker)
    : null;
  ...
  return (
    <div className="bg-indigo-600 text-white text-sm py-2 px-4 text-center transition-all duration-500">
      <Link
        ...
        className="hover:underline font-medium flex items-center justify-center gap-3 flex-wrap"
      >
        ...
        <span className="font-semibold">
          {match.battingTeam}: {score} ({overs} ov)
        </span>
