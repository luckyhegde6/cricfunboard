...
export default function MatchesTabs({
  ...
  const [tab, setTab] = useState<"all" | "live" | "scheduled" | "assigned">(
    "all",
  );
...
function TabButton({
  ...
  return (
    <button
      onClick={onClick}
      className={`px-3 py-1.5 rounded-md text-sm ${active ? "bg-slate-800 text-white" : "bg-white border text-slate-700"}`}
    >
      {label}
    </button>
  );
...
function AllTab({ matches }: { matches: MatchType[] }) {
  if (!matches.length)
    return <div className="text-sm text-slate-500">No matches yet.</div>;
  return (
...
function LiveTab({ matches }: { matches: MatchType[] }) {
  if (!matches.length)
    return (
      <div className="text-sm text-rose-500">No live matches right now.</div>
    );
  return (
    <div className="space-y-4">
      {matches.map((m) => (
        ...
        >
          <div>
            ...
            <div className="text-sm text-slate-500">
              {m.venue ?? "Venue unknown"}
            </div>
          </div>
          <div className="text-right">
            <div className="text-xl font-bold">
              {m.summary?.runs ?? 0}/{m.summary?.wickets ?? 0}
            </div>
            <div className="text-sm text-slate-500">
              {m.summary?.overs ?? 0} overs
            </div>
          </div>
        </div>
      ))}
...
function ScheduledTab({ matches }: { matches: MatchType[] }) {
  if (!matches.length)
    return <div className="text-sm text-slate-500">No scheduled matches.</div>;
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {matches.map((m) => (
        <MatchCard key={m._id} match={m} />
...
function AssignedTab({ matches }: { matches: MatchType[] }) {
  if (!matches.length)
    return <div className="text-sm text-slate-500">No assigned matches.</div>;
  return (
    <div className="space-y-4">
