...
export default function UserForm({ user, onClose }: { user?: any; onClose: () => void }) {
        ...
        try {
            ...
            if (user) {
                res = await fetch("/api/admin/users", {
                    ...
                    body: JSON.stringify({ id: user._id, email, name, role, ...(password ? { password } : {}) }),
                });
            } else {
                res = await fetch("/api/admin/users", {
                    method: "POST",
                    headers: { "content-type": "application/json" },
                    body: JSON.stringify({ email, name, role, password: password || "password123" }),
                });
            }
                ...
                <div className="space-y-4">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">
                            Password <span className="text-slate-400 font-normal text-xs ml-1">{user ? "(leave blank to keep current)" : "(defaults to 'password123')"}</span>
                        </label>
                        <input
                            type="password"
                            value={password}
