...
export default function TeamForm({ initialData }: { initialData?: TeamFormData }) {
    ...
    const [players, setPlayers] = useState<Player[]>(
        initialData?.players || [
            { name: "", role: "batsman", isExtra: false },
        ]
    );
        ...
        <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl mx-auto p-6 bg-white rounded-lg shadow">
            ...
            <div className="space-y-4">
                <div className="flex justify-between items-center">
                    ...
                    <button type="button" onClick={addPlayer} className="text-sm text-indigo-600 hover:text-indigo-800 font-medium">
                        + Add Player
                    </button>
                </div>

                {players.map((player, index) => (
                    <div key={index} className="flex flex-col md:flex-row gap-3 p-3 border rounded bg-slate-50 items-start md:items-center">
                        ...
                        <div className="flex items-center gap-2">
                            <label className="flex items-center gap-2 text-sm text-gray-600 cursor-pointer">
                                <input
                                    ...
                                    onChange={(e) => updatePlayer(index, "isExtra", e.target.checked)}
