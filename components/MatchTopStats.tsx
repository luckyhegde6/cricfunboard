...
export default function MatchTopStats({
  match: initialMatch,
  teamAPlayers,
  teamBPlayers,
}: {
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        ...
        <div className="flex-1 min-w-0">
          ...
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white truncate">
            {teamA}{" "}
            <span className="text-slate-400 text-xl font-normal mx-1">vs</span>{" "}
            {teamB}
          </h2>
        ...
        <div className="flex-1 text-center border-l border-r border-slate-100 dark:border-slate-700 px-4 md:px-8 min-w-0">
          ...
          <div className="text-lg text-slate-500 dark:text-slate-400 font-medium mt-1">
            {overs} overs
          </div>
          ...
          {lastOver.length > 0 && (
            <div className="flex flex-col items-end gap-2">
              ...
              <div
                ...
                data-testid="recent-balls"
              >
                {lastOver.map((ball: string, idx: number) => (
                  <span
                    key={idx}
                    className={`text-xs font-mono w-7 h-7 flex items-center justify-center rounded-md shadow-sm border ${
                      ball === "W"
      ...
      {latestAnnouncement && (
        <div className="mt-6 pt-4 border-t border-slate-100 dark:border-slate-700">
          <div className="flex items-start gap-3 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-100 dark:border-blue-800">
            ...
            <div>
              <div className="text-xs font-semibold text-blue-700 dark:text-blue-300 uppercase tracking-wide mb-0.5">
                Announcement
              </div>
              <div className="text-sm text-slate-700 dark:text-slate-300">
                {latestAnnouncement.meta?.text || "Update"}
              </div>
