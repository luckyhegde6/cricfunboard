...
export default function ScorerPanel({
  ...
  useEffect(() => {
    ...
    matchId, // payload expected from server; pass to parent
    onUpdate,
  ]);
    ...
    <div className="bg-white rounded-md border p-4 space-y-3">
      ...
      <div className="grid grid-cols-4 gap-2">
        <button
          ...
          onClick={() => record({ type: "dot" })}
        >
          Dot
        </button>
        <button
          className="px-3 py-2 rounded bg-white border"
          onClick={() => record({ type: "runs", runs: 1 })}
        >
          1
        </button>
        <button
          className="px-3 py-2 rounded bg-white border"
          onClick={() => record({ type: "runs", runs: 2 })}
        >
          2
        </button>
        <button
          className="px-3 py-2 rounded bg-white border"
          onClick={() => record({ type: "runs", runs: 3 })}
        >
          3
        </button>

        <button
          className="col-span-2 px-3 py-2 rounded bg-white border"
          onClick={() => record({ type: "runs", runs: 4 })}
        >
          4
        </button>
        <button
          className="col-span-2 px-3 py-2 rounded bg-white border"
          onClick={() => record({ type: "runs", runs: 6 })}
        >
          6
        </button>

        <button
          className="px-3 py-2 rounded bg-rose-100 text-rose-700"
          onClick={() => record({ type: "wicket" })}
        >
          Wicket
        </button>
        <button
          className="px-3 py-2 rounded bg-yellow-100 text-yellow-700"
          onClick={() => record({ type: "nb" })}
        >
          No-ball
        </button>
        <button
          className="px-3 py-2 rounded bg-amber-100 text-amber-700"
          onClick={() => record({ type: "wd" })}
        >
          Wide
        </button>
        <button
          className="px-3 py-2 rounded bg-slate-50 border"
