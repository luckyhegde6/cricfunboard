...
export default function MatchCard({ match: initialMatch }: { match: any }) {
  ...
  const { teamA, teamB, venue, startTime, status, currentInnings, battingTeam, innings1Summary, innings2Summary, currentBatters, currentBowler, teamAPlayers, teamBPlayers, recentEvents } = match;

  // Get current innings summary
  const currentSummary = currentInnings === 1 ? innings1Summary : innings2Summary;
  const score = currentSummary ? `${currentSummary.runs}/${currentSummary.wickets}` : "-/-";
  ...
  return (
    <Link href={`/match/${match._id}`} className="block bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 hover:shadow-lg transition p-4">
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1">
          <div className="text-sm text-slate-500 dark:text-slate-400">{venue ?? "Unknown venue"}</div>
          <div className="mt-1 font-semibold text-lg dark:text-white">{teamA} <span className="text-slate-400">vs</span> {teamB}</div>
          <div className="text-sm text-slate-500 dark:text-slate-400 mt-1">{formattedDate}</div>
            ...
            <div className="mt-3 space-y-1">
              ...
              {strikerName && bowlerName && (
                <div className="text-xs text-slate-500 dark:text-slate-400">
                  {strikerName}* {nonStrikerName && `& ${nonStrikerName}`} • {bowlerName}
                </div>
              )}
              {lastOver.length > 0 && (
                <div className="flex items-center gap-1 mt-2">
                  <span className="text-xs text-slate-500 dark:text-slate-400 mr-1">This over:</span>
                  {lastOver.map((ball: string, idx: number) => (
                    <span
                      key={idx}
                      className={`text-xs font-mono w-6 h-6 flex items-center justify-center rounded ${ball === "W"
        ...
        <div className="flex flex-col items-end">
          <div className={`rounded-md px-3 py-2 text-center font-medium transition-colors duration-300 ${status === 'live' ? 'bg-rose-600 text-white' : 'bg-slate-800 dark:bg-slate-700 text-white'}`}>
            <div className="text-lg">{score}</div>
            <div className="text-xs opacity-80">{overs} ov</div>
          </div>
          <div className={`mt-2 text-xs font-medium flex items-center gap-1 ${status === "live" ? "text-rose-600" : "text-slate-500 dark:text-slate-400"}`}>
            {status === "live" && (
